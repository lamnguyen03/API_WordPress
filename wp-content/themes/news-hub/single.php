	...
	<main id="primary" class="site-main">
		...
		while ( have_posts() ) :
			...
			get_template_part( 'template-parts/content', 'single' );
				...
				the_post_navigation(
					array(
						'prev_text' => '<span class="nav-subtitle">' . esc_html__( 'Previous:', 'news-hub' ) . '</span> <span class="nav-title">%title</span>',
						'next_text' => '<span class="nav-subtitle">' . esc_html__( 'Next:', 'news-hub' ) . '</span> <span class="nav-title">%title</span>',
					)
				);

			$related_posts_enable = get_theme_mod( 'news_hub_enable_single_post_related_posts', true );
			...
			if ( is_singular( 'post' ) ) {
				...
				if ( $query->have_posts() ) :
					$related_title = get_theme_mod( 'news_hub_related_posts_title', __( 'Related Posts', 'news-hub' ) );
					?>
...
if ( news_hub_is_sidebar_enabled() ) {
	get_sidebar();
}
