<?php
if ( ! class_exists( 'News_Hub_Posts_Tabs_Widget' ) ) {

	/**
	 * Adds News_Hub_Posts_Tabs_Widget.
	 */
	class News_Hub_Posts_Tabs_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$news_hub_posts_tabs_widget_ops = array(
				'classname'   => 'widget adore-widget posts-tabs-widget',
				'description' => __( 'Retrive Posts Tabs Widgets', 'news-hub' ),
			);
			parent::__construct(
				'news_hub_posts_tabs_widget',
				__( 'News Hub: Posts Tabs Widget', 'news-hub' ),
				$news_hub_posts_tabs_widget_ops
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			$tab_button_label = ( ! empty( $instance['button_label'] ) ) ? $instance['button_label'] : __( 'Read More', 'news-hub' );
			...
			<div class="post-tabs-wrapper">
				<div class="post-tabs-head">
					<ul class="post-tabs">
						<li><a href="#tab-1" class="latest"><i class="far fa-clock"></i><?php echo esc_html( 'Latest Posts', 'news-hub' ); ?></a></li>
						<li><a href="#tab-2" class="latest inactive"><i class="fas fa-fire"></i><?php echo esc_html( 'Trending Posts', 'news-hub' ); ?></a></li>
					</ul>
				</div>
		...
		public function form( $instance ) {
			$tab_button_label = isset( $instance['button_label'] ) ? $instance['button_label'] : __( 'Read More', 'news-hub' );
			$tab_post_offset  = isset( $instance['offset'] ) ? absint( $instance['offset'] ) : '';
			?>
	<p>
		<label for="<?php echo esc_attr( $this->get_field_id( 'button_label' ) ); ?>"><?php esc_html_e( 'Read More Button Label:', 'news-hub' ); ?></label>
