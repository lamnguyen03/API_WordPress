<?php
if ( ! class_exists( 'News_Hub_Social_Widget' ) ) {
	/**
	 * Adds News Hub Social Widget.
	 */
	class News_Hub_Social_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$news_hub_social_widget = array(
				'classname'   => 'widget adore-widget social-widget style-2',
				'description' => __( 'Retrive Social Widget', 'news-hub' ),
			);
			parent::__construct(
				'news_hub_social_widget',
				__( 'News Hub: Social Widget', 'news-hub' ),
				$news_hub_social_widget
			);
		}
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'open_link_new_tab' ) ); ?>"><?php esc_html_e( 'Open Social link in New Tab', 'news-hub' ); ?>:</label>
				<input type="checkbox" id="<?php echo esc_attr( $this->get_field_id( 'open_link_new_tab' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'open_link_new_tab' ), 'news-hub' ); ?>"  <?php checked( $open_link_new_tab, true ); ?> />
			</p>
			<?php
			for ( $i = 1; $i <= 4; $i++ ) {
				...
				?>
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'social_icon_label' . '-' . $i ) ); ?>"><?php echo sprintf( esc_html__( 'Social Icon Label %d :', 'news-hub' ), $i ); ?></label>
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'link' . '-' . $i ) ); ?>"><?php echo sprintf( esc_html__( 'Link %d :', 'news-hub' ), $i ); ?></label>
		...
		public function update( $new_instance, $old_instance ) {
			...
			$instance['open_link_new_tab']  = news_hub_sanitize_checkbox( $new_instance['open_link_new_tab'] );
