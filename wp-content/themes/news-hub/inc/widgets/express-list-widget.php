<?php
if ( ! class_exists( 'News_Hub_Express_List_Widget' ) ) {
	/**
	 * Adds News Hub Express List Widget.
	 */
	class News_Hub_Express_List_Widget extends WP_Widget {
		...
		public function __construct() {
			$express_list_widget = array(
				...
				'description' => __( 'Retrive Express List Widgets', 'news-hub' ),
			);
			parent::__construct(
				'news_hub_express_list_widget',
				__( 'News Hub: Express List Widget', 'news-hub' ),
				$express_list_widget
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			$read_more_button_label   = ( ! empty( $instance['read_more_button_label'] ) ) ? $instance['read_more_button_label'] : __( 'Read More', 'news-hub' );
		...
		public function form( $instance ) {
			...
			$read_more_button_label   = isset( $instance['read_more_button_label'] ) ? $instance['read_more_button_label'] : __( 'Read More', 'news-hub' );
			$express_list_post_offset = isset( $instance['offset'] ) ? absint( $instance['offset'] ) : '';
			$express_list_category    = isset( $instance['category'] ) ? absint( $instance['category'] ) : '';
			?>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Section Title:', 'news-hub' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'view_all_button_label' ) ); ?>"><?php esc_html_e( 'View All Button:', 'news-hub' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'view_all_button_link' ) ); ?>"><?php esc_html_e( 'View All Button URL:', 'news-hub' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'read_more_button_label' ) ); ?>"><?php esc_html_e( 'Read More Button Label:', 'news-hub' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'offset' ) ); ?>"><?php esc_html_e( 'Number of posts to displace or pass over:', 'news-hub' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'category' ) ); ?>"><?php esc_html_e( 'Select the category to show posts:', 'news-hub' ); ?></label>
				<select id="<?php echo esc_attr( $this->get_field_id( 'category' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'category' ) ); ?>" class="widefat" style="width:100%;">
					<?php
					$categories = news_hub_get_post_cat_choices();
