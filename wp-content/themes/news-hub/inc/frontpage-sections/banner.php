...
// Banner Section.
$banner_section = get_theme_mod( 'news_hub_banner_section_enable', false );
...
$content_ids  = array();
$banner_content_type = get_theme_mod( 'news_hub_banner_content_type', 'post' );

if ( $banner_content_type === 'post' ) {

	for ( $i = 1; $i <= 4; $i++ ) {
		$content_ids[] = get_theme_mod( 'news_hub_banner_post_' . $i );
	}
	...
	$cat_content_id = get_theme_mod( 'news_hub_banner_category' );
	$args           = array(
		'cat'            => $cat_content_id,
		'posts_per_page' => absint( 4 ),
	);
...
if ( $query->have_posts() ) {
	?>
	<div id="news_hub_banner_section" class="main-banner-section style-4 adore-navigation">
